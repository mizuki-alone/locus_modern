...
## ポートフォリオ方針
このプロジェクトはUpworkフリーランスポートフォリオとしても使う。以下を常に意識すること。
- **UIテキストは英語**: ボタン、ラベル、プレースホルダー、ステータスメッセージ等すべて英語にする
- **コードコメントは英語**: ソースコード内のコメントも英語で書く
- **commit メッセージは英語**: git のコミットメッセージも英語にする
- **プロフェッショナルな見た目**: 細部の揃い、余白、フォント選択に気を配る
- **README.md**: 英語で書き、スクリーンショットやGIF動画を含めてデモが伝わるようにする
...
## アーキテクチャ方針
- **データ層を分離すること**: `route.ts` がファイル形式（memo.cgi）を直接知らない設計を目指す。将来SQLite等への移行を容易にするため、データアクセス層を間に挟む
- 現段階では大規模なリファクタリングは不要。新機能追加時に意識する程度でよい
